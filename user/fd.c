...
int fd_alloc(struct Fd **fd) {
#ifdef DEBUG
        writef("fd_alloc@fd.c called to alloc a new struct Fd\n");
#endif
        u_int va;
        u_int fdno;
        ...
        for (fdno = 0; fdno < MAXFD - 1; fdno++) {
                ...
                if (((* vpd)[va / PDMAP] & PTE_V) == 0) {
                        *fd = (struct Fd *)va;
#ifdef DEBUG
                        writef("fd_alloc@fd.c: alloced struct Fd#%x at %x\n", fdno, va);
#endif
                        return 0;
                }
...
#ifdef DEBUG
        writef("read@fd.c called with (int fdnum: %x, void *buf %x, u_int n: %x)\n", fdnum, buf, n);
#endif
        int r;
        struct Dev *dev;
        struct Fd *fd;
