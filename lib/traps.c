...
#ifdef DEBUG
        printf("page_fault_handler@traps.c called\n");
#endif
        struct Trapframe PgTrapFrame;
        extern struct Env *curenv;

        bcopy(tf, &PgTrapFrame, TF_SIZE);

        if (tf->regs[29] >= (curenv->env_xstacktop - BY2PG) &&
                        tf->regs[29] <= (curenv->env_xstacktop - 1)) {
                tf->regs[29] = tf->regs[29] - TF_SIZE;
                bcopy(&PgTrapFrame, (void *)tf->regs[29], TF_SIZE);
        } else {
                tf->regs[29] = curenv->env_xstacktop - TF_SIZE;
                bcopy(&PgTrapFrame, (void *)curenv->env_xstacktop - TF_SIZE, TF_SIZE);
        }
