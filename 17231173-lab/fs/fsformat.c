...
static inline void print_struct_File(struct File *fp) {
        printf("{f_name=%s, f_size=%x, f_type=%x, ...}", fp->f_name, fp->f_size, fp->f_type);
}
...
void init_disk(void) {
#ifdef DEBUG
        printf("init_disk@fsformat.c called\n");
#endif
    int i, r, diff;
...
#ifdef DEBUG
        printf("init_disk@fsformat.c: marked disk[0].type as boot\n");
#endif
...
#ifdef DEBUG
                printf("init_disk@fsformat.c: marked disk[%x].type as bmap\n", i+2);
#endif
    }
    for(i = 0; i < nbitblock; ++i) {
...
#ifdef DEBUG
        printf("init_disk@fsformat.c: marked disk[1].type as super\n");
#endif
...
#ifdef DEBUG
        printf("init_disk@fsformat.c: initiated super to {s_magic=%x, s_nblocks=%x, ...}\n", (int) super.s_magic, (int) super.s_nblocks);
#endif
}
...
void flush_bitmap() {
#ifdef DEBUG
        printf("flush_bitmap@fsformat.c: updating bitmap to mark all bit where corresponding block is used\n");
#endif
    int i;
...
#ifdef DEBUG
        printf("finish_fs@fsformat.c: writing back to %s\n", name);
#endif
    int fd, i, k, n, r;
    uint32_t *p;
...
#ifdef DEBUG
        printf("save_block_link@fsformat.c: linked block#%x of struct File ", nblk);
        print_struct_File(f);
        printf(" to bno %x\n", bno);
#endif
}
...
struct File *create_file(struct File *dirf) {
#ifdef DEBUG
        printf("create_file@fsformat.c called with (struct File *dirf: ");
        print_struct_File(dirf);
        printf(")\n");
#endif
    struct File *dirblk;
    int i, bno, found;
    int nblk = dirf->f_size / BY2BLK;
        ...
        // already contains blocks
        // traverse struct Files, find an empty one
        if (nblk != 0) {
                if (nblk <= NDIRECT) {
...
#ifdef DEBUG
                printf("create_file@fsformat.c: found last block#%x, at bno %x\n", nblk-1, bno);
#endif
...
#ifdef DEBUG
        printf("create_file@fsformat.c: no space for new file, applying for new block\n");
#endif
...
#ifdef DEBUG
        printf("write_file@fsformat.c called with (struct File *dirf: ");
        print_struct_File(dirf);
...
#ifdef DEBUG
        printf("\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
        printf("main@fsformat.c called with (int argc: %x, char **argv: ", argc);
        for (i = 0; i < argc - 1; i++) {
                printf("%s, ", argv[i]);
        }
        printf("%s)\n", argv[i]);
