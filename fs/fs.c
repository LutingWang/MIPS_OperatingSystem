...
#ifdef DEBUG
        // writef("diskaddr@fs.c called with (u_int blockno: %x)\n", blockno);
#endif
        if (super) {
                if (blockno > super->s_nblocks) { // 1024
                        user_panic("diskaddr@fs.c: blockno %x is greater than nblocks %x\n", blockno, super->s_nblocks);
                }
        }
#ifdef DEBUG
        // writef("diskaddr@fs.c: addr is %x\n", DISKMAP + blockno * BY2BLK);
#endif
...
#ifdef DEBUG
        writef("map_block@fs.c called with (u_int blockno: %x)\n", blockno);
#endif
...
#ifdef DEBUG
        writef("unmap_block@fs.c called with (u_int blockno %x)\n", blockno);
#endif
        // int r;
        if (!block_is_mapped(blockno)) {
#ifdef DEBUG
                writef("unmap_block@fs.c: block is mapped, returnning\n");
#endif
                return;
        }
...
#ifdef DEBUG
        writef("read_bitmap@fs.c called\n");
#endif
        u_int i;
        void *blk = NULL;
...
#ifdef DEBUG
        writef("check_write_block@fs.c called\n");
#endif
        super = 0;
...
#ifdef DEBUG
        writef("check_write_block@fs.c over\n");
#endif
}
...
#ifdef DEBUG
        writef("dir_lookup@fs.c called with (struct File *dir: %x, char *name: %x(%s), struct File **file)\n", dir, name, name);
#endif
        int r;
        u_int i, j, nblock;
        void *blk;
...
/* this is an internal function used only in DEBUG mode*/
static void dir_traverse(void) {
        writef("dir_traverse@fs.c called to print dir hierachy\n");
        int r;
        u_int i, j, nblock;
        void *blk;
        struct File *f, *dir;
        u_char path[MAXNAMELEN];
        dir = &super->s_root;
        nblock = dir->f_size / BY2BLK;
        user_bcopy(dir->f_name, path, MAXNAMELEN);

        writef("%s\n", path);
        for (i = 0; i < nblock; i++) {
                if ((r = file_get_block(dir, i, &blk))) {
                        writef("dir_traverse@fs.c: file get block failed\n");
                        return;
                }
                f = (struct File *) blk;
                for (j = 0; j < FILE2BLK; j++) {
